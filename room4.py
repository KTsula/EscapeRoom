...
def room4(win, inventory):
    ...
    # drawing room
    img4 = Image(Point(20, 10), "room4.gif")
    img4.draw(win)

    # drawing user
    user = Image(Point(29.0,6.0), "thief.gif")
    user.draw(win)
    ...
    while continueGame is True and lost is False:
        ...
        if k == 'O' or k == 'o':
            if observe.active:
                test_passed = test_code(win, observed_display)
        ...
        if test_passed and main_item in inventory:
            ...
            storytell(win,"Good Job! You passed the fourth room because the riddle you solved activated the remote control and you were able to unlock a hidder door under the bed.")
            user.undraw()
            img4.undraw()
